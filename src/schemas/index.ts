...
export const ListContainersSchema = z
  .object({
    ...hostFilterSchema,
    state: z
      .enum(["all", "running", "stopped", "paused"])
      .default("all")
      .describe("Filter by container state"),
    name_filter: z.string().optional().describe("Filter containers by name (partial match)"),
    image_filter: z.string().optional().describe("Filter by image name (partial match)"),
    label_filter: z
      .string()
      .optional()
...
export const ContainerActionSchema = z
  .object({
    container_id: z.string().min(1).describe("Container ID or name"),
    host: z
      .string()
      .optional()
...
export const GetLogsSchema = z
  .object({
    ...
    grep: z.string().optional().describe("Filter logs containing this string (case-insensitive)"),
    stream: z
...
export const ContainerStatsSchema = z
  .object({
    ...
    host: z.string().optional().describe("Host to get stats from"),
    ...responseFormatSchema
  })
  .strict();
...
export const InspectContainerSchema = z
  .object({
    container_id: z.string().min(1).describe("Container ID or name"),
    host: z
      .string()
...
export const HostStatusSchema = z
  .object({
    host: z.string().optional().describe("Specific host to check (omit for all hosts)"),
    ...responseFormatSchema
  })
  .strict();
...
export const SearchContainersSchema = z
  .object({
    query: z
      .string()
      .min(1)
...
// Compose project action schema
export const ComposeProjectSchema = z
  .object({
    project: z.string().min(1).describe("Docker Compose project name"),
    host: z.string().min(1).describe("Host where project is running"),
    action: z
      .enum(["status", "up", "down", "restart", "logs"])
...
// List compose projects schema
export const ListComposeProjectsSchema = z
  .object({
    host: z.string().min(1).describe("Host to list compose projects from"),
    ...responseFormatSchema
  })
  .strict();
export type ListComposeProjectsInput = z.infer<typeof ListComposeProjectsSchema>;

// Compose logs schema
export const ComposeLogsSchema = z
  .object({
    project: z.string().min(1).describe("Docker Compose project name"),
    host: z.string().min(1).describe("Host where project is running"),
    service: z.string().optional().describe("Specific service to get logs from (omit for all)"),
    lines: z
      .number()
      .int()
      .min(1)
...
// List images schema
export const ListImagesSchema = z
  .object({
    ...hostFilterSchema,
    dangling_only: z.boolean().default(false).describe("Only show dangling (untagged) images"),
    ...paginationSchema,
    ...responseFormatSchema
  })
  .strict();
...
export const PruneSchema = z
  .object({
    host: z.string().optional().describe("Host to prune (omit for all hosts)"),
    target: z
      .enum(["containers", "images", "volumes", "networks", "buildcache", "all"])
      .describe("What to prune"),
    force: z.boolean().default(false).describe("Confirm destructive operation (required)")
  })
  .strict();
...
export const DockerInfoSchema = z
  .object({
    host: z.string().optional().describe("Host to get Docker info from (omit for all hosts)"),
    ...responseFormatSchema
  })
  .strict();
...
export const DockerDiskUsageSchema = z
  .object({
    host: z.string().optional().describe("Host to get disk usage from (omit for all hosts)"),
    ...responseFormatSchema
  })
  .strict();
...
export const HostResourcesSchema = z
  .object({
    host: z.string().optional().describe("Host to get resources from (omit for all hosts)"),
    ...responseFormatSchema
  })
  .strict();
