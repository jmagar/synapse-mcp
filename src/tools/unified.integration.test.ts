...
// Mock the loadHostConfigs module function
...
const createMockDockerService = (): IDockerService => ({
  ...
  getHostStatus: vi.fn().mockResolvedValue([]),
  loadHostConfigs: vi.fn().mockReturnValue([
    { name: "testhost", host: "localhost", port: 2375, protocol: "http" } as HostConfig
  ]),
  getDockerClient: vi.fn()
});

const createMockComposeService = (): IComposeService => ({
  composeExec: vi.fn().mockResolvedValue("command output"),
  ...
  describe("container actions", () => {
    ...
    describe("container action: stats", () => {
      it("should get container stats for single host", async () => {
        vi.spyOn(mockDockerService, "getContainerStats").mockResolvedValue({
          containerId: "abc123",
          containerName: "my-container",
          cpuPercent: 25.5,
      ...
      it("should get container stats across all hosts when host not specified", async () => {
        ...
        expect(mockDockerService.findContainerHost).toHaveBeenCalledWith(
          "my-container",
          expect.any(Array)
        );
        expect(result.content).toBeDefined();
    ...
    describe("container action: inspect", () => {
      it("should inspect container with summary mode (default)", async () => {
        vi.spyOn(mockDockerService, "inspectContainer").mockResolvedValue({
          Id: "abc123456789",
          Name: "/my-container",
          Config: {
            Image: "nginx:latest",
        ...
        expect(mockDockerService.inspectContainer).toHaveBeenCalledWith(
          "my-container",
          expect.objectContaining({ name: "testhost" })
        );
  ...
  describe("compose actions", () => {
    ...
    describe("compose action: list", () => {
      it("should list all compose projects", async () => {
        ...
        expect(mockComposeService.listComposeProjects).toHaveBeenCalledWith(
          expect.objectContaining({ name: "testhost" })
        );
        expect(result.content).toBeDefined();
    ...
    describe("compose action: status", () => {
      it("should get compose project status", async () => {
        ...
        expect(mockComposeService.getComposeStatus).toHaveBeenCalledWith(
          expect.objectContaining({ name: "testhost" }),
          "myapp"
        );
        expect(result.content).toBeDefined();
    ...
    describe("compose action: lifecycle operations", () => {
      ...
      it("should restart compose project", async () => {
        ...
        expect(mockComposeService.composeRestart).toHaveBeenCalledWith(
          expect.objectContaining({ name: "testhost" }),
          "myapp"
        );
        expect(result.content).toBeDefined();
  ...
  describe("host actions", () => {
    ...
    describe("host action: resources", () => {
      it("should get host resources", async () => {
        ...
        vi.spyOn(mockSSHService, "getHostResources").mockResolvedValue({
        ...
        expect(mockSSHService.getHostResources).toHaveBeenCalledWith(
          expect.objectContaining({ name: "testhost" })
        );
        expect(result.content).toBeDefined();
        expect(result.content[0].text).toContain("CPU");
  ...
  describe("docker actions", () => {
    describe("docker action: info", () => {
      it("should get Docker system info", async () => {
        vi.spyOn(mockDockerService, "getDockerInfo").mockResolvedValue({
        ...
        expect(mockDockerService.getDockerInfo).toHaveBeenCalledWith(
          expect.objectContaining({ name: "testhost" })
        );
        expect(result.content).toBeDefined();
    ...
    describe("docker action: df", () => {
      it("should get Docker disk usage", async () => {
        vi.spyOn(mockDockerService, "getDockerDiskUsage").mockResolvedValue({
          images: {
            active: 10,
        ...
        expect(mockDockerService.getDockerDiskUsage).toHaveBeenCalledWith(
          expect.objectContaining({ name: "testhost" })
        );
        expect(result.content).toBeDefined();
    ...
    describe("docker action: prune", () => {
      ...
      it("should prune containers with force flag", async () => {
        vi.spyOn(mockDockerService, "pruneDocker").mockResolvedValue([
          {
            type: "containers",
      ...
      it("should handle errors during prune operation", async () => {
        vi.spyOn(mockDockerService, "pruneDocker").mockRejectedValue(
  ...
  describe("image actions", () => {
    ...
    describe("image action: list", () => {
      it("should list all images", async () => {
        vi.spyOn(mockDockerService, "listImages").mockResolvedValue([
          {
            id: "sha256:abc123",
            tags: ["nginx:latest"],
            size: 142 * 1024 * 1024,
...
describe("Container stats collection performance", () => {
  it("should measure baseline performance (was sequential, now parallel)", async () => {
    // Create custom mock with performance test data
    const perfMockDocker = createMockDockerService();
    perfMockDocker.getContainerStats = vi.fn().mockImplementation(async (id, _host) => {
    ...
    perfMockDocker.listContainers = vi.fn().mockResolvedValue(
    ...
    perfMockDocker.loadHostConfigs = vi.fn().mockReturnValue([
      {
        name: "host1",
        host: "192.168.1.10",
    ...
    const container: ServiceContainer = {
      getDockerService: () => perfMockDocker,
      setDockerService: vi.fn(),
      getSSHConnectionPool: vi.fn(),
      setSSHConnectionPool: vi.fn(),
      getSSHService: () => createMockSSHService(),
      setSSHService: vi.fn(),
      getComposeService: () => createMockComposeService(),
      setComposeService: vi.fn(),
      getFileService: () => createMockFileService(),
      setFileService: vi.fn(),
      cleanup: vi.fn().mockResolvedValue(undefined)
    } as unknown as ServiceContainer;
  ...
  it("should collect stats in parallel across hosts and containers", async () => {
    // Update module-level loadHostConfigs to return 2 hosts for this test
    const { loadHostConfigs } = await import("../services/docker.js");
    const originalMock = vi.mocked(loadHostConfigs).getMockImplementation();
    vi.mocked(loadHostConfigs).mockReturnValue([
      { name: "host1", host: "192.168.1.10", protocol: "http" as const, port: 2375 },
      { name: "host2", host: "192.168.1.11", protocol: "http" as const, port: 2375 }
    ]);
    ...
    perfMockDocker.listContainers = vi.fn().mockImplementation(async (hosts) => {
      // Return 5 containers for each host (10 total across 2 hosts)
      return hosts.flatMap((host) =>
        Array.from({ length: 5 }, (_, i) => ({
          id: `container-${host.name}-${i}`,
          name: `test-${host.name}-${i}`,
          image: "test:latest",
          state: "running" as const,
          status: "Up 1 hour",
          created: new Date().toISOString(),
    ...
    if (originalMock) {
      vi.mocked(loadHostConfigs).mockImplementation(originalMock);
    }
  }, 10000);

  it("should handle partial failures gracefully", async () => {
    // Create custom mock with failure scenarios
    const perfMockDocker = createMockDockerService();
    perfMockDocker.getContainerStats = vi.fn().mockImplementation(async (id, _host) => {
      if (id === "container-2") {
...
describe("Error handling: invalid hosts", () => {
  ...
  beforeEach(async () => {
    ...
    vi.spyOn(mockDockerService, "loadHostConfigs").mockReturnValue([]);
  });
...
describe("Edge cases: empty results", () => {
  ...
  it("should handle container search with no results", async () => {
    ...
    vi.spyOn(mockDockerService, "findContainerHost").mockResolvedValue(null);
  ...
  it("should handle empty compose project list", async () => {
    vi.spyOn(mockComposeService, "listComposeProjects").mockResolvedValue([]);
  ...
  it("should handle pagination with offset beyond results", async () => {
    vi.spyOn(mockDockerService, "listContainers").mockResolvedValue([
      {
        id: "abc123",
        name: "test-container",
        image: "nginx:latest",
  ...
  it("should handle empty logs output", async () => {
    vi.spyOn(mockDockerService, "getContainerLogs").mockResolvedValue([]);
